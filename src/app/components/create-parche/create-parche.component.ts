...
export class CreateParcheComponent {
  ...
  async onCreate(): Promise<void> {
    ...
    try {
      await firstValueFrom(this.apiService.createParche({
        Nombre: this.parche.name.trim(),
        Descripcion: this.parche.description.trim() || undefined
      }));

      alert('¡Parche creado exitosamente!');
      this.parche = { name: '', description: '', coverImageUrl: '' };
      this.coverPreviewUrl = '';
    } catch (error: any) {
      this.errorMessage = error?.error?.error ?? error?.message ?? 'Error al crear el parche. Intenta de nuevo.';
    } finally {
